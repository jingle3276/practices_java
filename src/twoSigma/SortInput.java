...
public class SortInput {
	...
	public static String sort (String str){
		...
		//scan through the string build up the two queues. 
		while ( st.hasMoreTokens() )
		{
		  String token = (String)st.nextToken();
		
		  if(isInt(token)){
			  intQueue.add(Integer.parseInt(token));
		  }
		...
		//scan again and build the output
		while ( st.hasMoreTokens() )
		{
		  String token = (String)st.nextToken();
	...
	public static void main(String args[]){
		...
		String input = ir.getInput("enter the input string:");
		
		System.out.println(SortInput.sort(input));
